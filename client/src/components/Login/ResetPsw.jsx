...
function ResetPsw() {
  ...
  async function submit(e) {
    ...
    try {
      setErrs(
        validation({ password: password, repeatPassword: repeatPassword })
      );
      if (indicator) {
      ...
      await axios
        .post(`${baseURL}/forgetpswUsers?id=${id}&token=${token}`, {
          password,
          repeatPassword,
        })
        .then((res) => {
          ...
          if (res.data === "wrong length") {
            setErrs({
              password: "Your password should in length from 8 to 15",
            });
          }
          ...
          if (res.data === "psw reset") {
            setMsg(
              "Your psw has been reset successfully, please go to log in page"
            );
            console.log(msg);
          }
        });
    ...
    <div className="login-container">
      {validUrl ? (
        <form action="POST" onSubmit={submit}>
          ...
          <button type="submit" className="btn btn-green" id="send-reset">
            reset password
          </button>
        </form>
      ) : (
        <h1>
          404 Not Found. The link has expired, Go back to{" "}
          <Link to="/forgetpsw" className="login-link">
            Forget Password
          </Link>{" "}
          page and resend the link
        </h1>
      )}
    </div>
  );
